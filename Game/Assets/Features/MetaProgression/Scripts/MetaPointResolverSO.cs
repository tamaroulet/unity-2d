...
namespace Game.Features.MetaProgression
{
    ...
    public class MetaPointResolverSO : ScriptableObject
    {
        ...
        {
            var rules = new MetaPointRules(_turnBonusPerTurn, _skillBonusMultiplier, _bossDefeatedBonus, _gameClearBonus);
            return rules.CalculateEarnedPoints(finalState, isGameClear, bossDefeatedCount);
        }
        ...
        {
            var rules = new MetaPointRules(_turnBonusPerTurn, _skillBonusMultiplier, _bossDefeatedBonus, _gameClearBonus);
            return rules.ApplyRunResult(currentProfile, earnedPoints);
        }
